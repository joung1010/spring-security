...
```java 
        http
                ...
                .csrf(AbstractHttpConfigurer::disable);
```
`AuthorizationManager` 클레스 `check()` 메서드를 통해 실제 인가에 대한 처리를 위임받아 처리한다.
...
// 생성자를 통한 불변 객체 생성
    private AuthorityAuthorizationManager(Set<String> authorities) {
        this.authorities = Collections.unmodifiableSet(new LinkedHashSet<>(authorities));
        this.roleHierarchy = null;
        this.grantedAuthorityDefaults = new GrantedAuthorityDefaults();
    }
}

```

- `AuthorityAuthorizationManager`는 `AuthorizationManager<RequestAuthorizationContext>` 인터페이스의 구현체로, 권한 기반 인가 결정을 담당합니다.
- `Set<String> authorities`는 검증해야 할 권한 목록을 불변 컬렉션으로 저장하여 thread-safety를 보장합니다.
...
// 3단계: 권한 매칭 로직 위임
    boolean granted = hasRequiredAuthorities(auth);
...
- `Supplier<Authentication>`을 통한 **지연 로딩(Lazy Loading)**: 실제로 필요할 때만 인증 객체를 가져와 성능을 최적화합니다.
- **조기 반환 패턴**: 인증되지 않은 경우 즉시 false를 반환하여 불필요한 권한 검사를 방지합니다.
- **단일 책임 원칙**: `hasRequiredAuthorities()` 메서드에 실제 권한 검증 로직을 위임합니다.
- `AuthorizationDecision`은 불변 객체로 인가 결과를 안전하게 캡슐화합니다.

### 권한 검증 로직의 구체적인 위임 과정
...
- **사용자 권한 추출**: `Authentication.getAuthorities()`로 현재 사용자의 모든 권한을 가져옵니다.
- **계층적 권한 확장**: `RoleHierarchy`가 설정된 경우, 상위 권한으로부터 도달 가능한 모든 권한을 계산합니다.
- **조건부 처리**: 계층이 없는 경우 원본 권한을 그대로 사용하여 불필요한 처리를 방지합니다.
...
private boolean matchAuthorities(Collection<? extends GrantedAuthority> userAuthorities) {
    Set<String> userAuthorityStrings = userAuthorities.stream()
            .map(GrantedAuthority::getAuthority)
            .filter(Objects::nonNull)// null 권한 필터링
            .collect(Collectors.toSet());

// 교집합 존재 여부 확인 (효율적인 O(1) 평균 검색)
...
### 역할(Role) 처리의 특별한 위임 구조

```java

public static AuthorityAuthorizationManager hasRole(String role) {
// 역할을 권한 형태로 변환하여 위임
    String roleAuthority = toRoleAuthority(role);
    return new AuthorityAuthorizationManager(Set.of(roleAuthority));
}

public static AuthorityAuthorizationManager hasAnyRole(String... roles) {
// 다중 역할을 권한 집합으로 변환하여 위임
    Set<String> roleAuthorities = Arrays.stream(roles)
            .map(AuthorityAuthorizationManager::toRoleAuthority)
...
### AuthorizationDecision 생성과 반환

```java

private AuthorizationDecision createDecision(boolean granted, Authentication auth, RequestAuthorizationContext context) {
// 기본 결정 객체 생성
    AuthorizationDecision decision = new AuthorizationDecision(granted);

// 디버그 정보 로깅 (성능 고려)
    if (logger.isDebugEnabled()) {
        logAuthorizationDetails(auth, granted, context);
    }

// 메트릭 수집 (선택적)
    if (metricsEnabled) {
        recordAuthorizationMetrics(granted, context);
    }
...
public final class AuthorityAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
    ...
    public AuthorizationDecision check(Supplier<Authentication> authentication, RequestAuthorizationContext object) {
        try {
// 위임 체인 시작
            return performAuthorizationCheck(authentication, object);
        } catch (Exception e) {
// 예외 상황 처리
            logger.warn("Authorization check failed", e);
            return new AuthorizationDecision(false);
        }
    }

    private AuthorizationDecision performAuthorizationCheck(Supplier<Authentication> authentication,
                                                          RequestAuthorizationContext object) {
// 1. 인증 객체 획득
        Authentication auth = retrieveAuthentication(authentication);

// 2. 기본 검증 위임
        if (!isValidForAuthorization(auth)) {
            return new AuthorizationDecision(false);
        }

// 3. 권한 검증 위임
        boolean hasAuthority = delegateAuthorityCheck(auth);

// 4. 컨텍스트 검증 위임 (선택적)
        boolean contextValid = delegateContextCheck(object);

// 5. 최종 결정
        boolean granted = hasAuthority && contextValid;
        return new AuthorizationDecision(granted);
    }

    private Authentication retrieveAuthentication(Supplier<Authentication> supplier) {
        return supplier.get();
    }

    private boolean isValidForAuthorization(Authentication auth) {
        return auth != null &&
               auth.isAuthenticated() &&
               !(auth instanceof AnonymousAuthenticationToken);
    }

    private boolean delegateAuthorityCheck(Authentication auth) {
        return hasRequiredAuthorities(auth);
    }

    private boolean delegateContextCheck(RequestAuthorizationContext context) {
// IP 주소, 시간, 기타 컨텍스트 기반 검증
        return true;// 기본 구현에서는 항상 true
    }
}
