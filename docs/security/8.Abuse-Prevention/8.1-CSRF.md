...
- **세션 관리와 SameSite:**
  ...
  Spring Security의 HttpSecurity 설정 내에서 별도의 쿠키 설정은 없더라도, Spring Session Core에서 정의한 CookieSerializer 빈이 활성화되면 해당 설정이 적용되어 세션 쿠키에 반영됩니다.
